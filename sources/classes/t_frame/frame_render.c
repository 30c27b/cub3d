...
/*   Updated: 2021/01/11 12:14:09 by ancoulon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (screen_x < game->map->res_x)
	{
		...
		while (screen_y<game->map->res_y)
		{
			...
			else if (screen_y >= drawStart && screen_y <= drawEnd)
			{
				...
				texPos += step;
				frame_put_pixel(frame, vect_init(screen_x, screen_y),
				texture_get_pixel(ray->wall_tex, vect_init(tex_x, tex_y)));
			}
			else
